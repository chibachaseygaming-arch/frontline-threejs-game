...
type Bot = { mesh: THREE.Group; team: "RED" | "BLUE"; hp: number; target: THREE.Vector3; cooldown: number; skill: keyof typeof DIFFICULTIES; classKey: ClassKey; bar: THREE.Mesh };
...
export default function Game() {
  ...
  useEffect(() => {
    ...
    const addBox = (x:number,z:number,w:number,h:number,d:number,color=0x5d6058) => { const m = new THREE.Mesh(new THREE.BoxGeometry(w,h,d), new THREE.MeshStandardMaterial({color,roughness:.82})); m.position.set(x,h/2,z); m.castShadow=true;m.receiveShadow=true;scene.add(m);obstacles.push(m); };
    ...
    function botMesh(team:"RED"|"BLUE", classKey:ClassKey){
      ...
      const head=part(new THREE.SphereGeometry(.29,16,12),skin,0,2.14,0);head.scale.set(.92,1.05,.92);part(new THREE.SphereGeometry(.32,16,10,0,Math.PI*2,0,Math.PI*.62),uniform,0,2.23,0);part(new THREE.BoxGeometry(.48,.13,.09),dark,0,2.17,.27);part(new THREE.BoxGeometry(.58,.1,.18),armor,0,1.91,0);
      ...
      const bg=new THREE.Mesh(new THREE.PlaneGeometry(1.2,.12),new THREE.MeshBasicMaterial({color:0x1c211e,side:THREE.DoubleSide}));bg.position.set(0,2.75,0);g.add(bg);const bar=new THREE.Mesh(new THREE.PlaneGeometry(1.16,.08),new THREE.MeshBasicMaterial({color:team==="RED"?0xf05a4a:0x58a9ef,side:THREE.DoubleSide}));bar.position.set(0,2.75,.005);g.add(bar);return {g,bar};
    }
    function spawnBots(){bots.splice(0).forEach(()=>{});for(let i=0;i<14;i++){const team=i<8?"RED":"BLUE", cm=botMesh(team,CLASS_KEYS[i%5]);cm.g.position.set(team==="RED"?-75+Math.random()*12:70+Math.random()*12,0,(Math.random()-.5)*70);scene.add(cm.g);bots.push({mesh:cm.g,bar:cm.bar,team,hp:200,target:points[i%3].pos.clone(),cooldown:Math.random(),skill:difficultyRef.current,classKey:CLASS_KEYS[i%5]});}}
    ...
    const gun=new THREE.Group(),weaponModels={} as Record<ClassKey,THREE.Group>;const gunMetal=new THREE.MeshStandardMaterial({color:0x202526,metalness:.72,roughness:.3}),gunDark=new THREE.MeshStandardMaterial({color:0x090b0c,metalness:.45,roughness:.46}),gripMat=new THREE.MeshStandardMaterial({color:0x161918,roughness:.8}),handMat=new THREE.MeshStandardMaterial({color:0xb98562,roughness:.82}),sleeveMat=new THREE.MeshStandardMaterial({color:0x30483b,roughness:.9});
    const makeViewWeapon=(key:ClassKey)=>{const w=new THREE.Group();const box=(sx:number,sy:number,sz:number,x:number,y:number,z:number,mat:THREE.Material=gunMetal)=>{const m=new THREE.Mesh(new THREE.BoxGeometry(sx,sy,sz),mat);m.position.set(x,y,z);m.castShadow=true;w.add(m);return m};const tube=(radius:number,length:number,x:number,y:number,z:number,mat:THREE.Material=gunDark)=>{const m=new THREE.Mesh(new THREE.CylinderGeometry(radius,radius,length,10),mat);m.rotation.x=Math.PI/2;m.position.set(x,y,z);m.castShadow=true;w.add(m);return m};
      if(key==="LIGHT"){box(.3,.24,.72,.25,-.28,-.72);tube(.045,.5,.25,-.25,-1.32);box(.14,.42,.16,.25,-.49,-.66,gripMat);box(.08,.18,.56,.25,-.27,-.2,gunDark);}
      else if(key==="MEDIUM"){box(.32,.25,.78,.25,-.28,-.72);box(.25,.22,.58,.25,-.26,-1.35,gunDark);tube(.045,.75,.25,-.25,-2);box(.18,.46,.2,.25,-.51,-.77,gripMat).rotation.x=-.18;box(.14,.38,.22,.25,-.46,-1.02,gunDark).rotation.x=.2;}
      else if(key==="HEAVY"){tube(.24,.65,.22,-.25,-.67,gunMetal);for(let i=0;i<6;i++){const a=i/6*Math.PI*2;tube(.035,1.35,.22+Math.cos(a)*.14,-.25+Math.sin(a)*.14,-1.65,gunDark);}tube(.29,.18,.22,-.25,-1.05,gunMetal);tube(.22,.16,.22,-.25,-2.28,gunMetal);box(.28,.18,.38,.22,-.47,-.49,gripMat);}
      else if(key==="SCOUT"){box(.28,.24,1.08,.25,-.28,-.92);tube(.04,1.35,.25,-.25,-2.08);box(.2,.38,.22,.25,-.48,-.72,gripMat);box(.12,.16,.7,.25,-.29,-.18,gripMat);tube(.105,.58,.25,-.08,-.86,gunDark);tube(.055,.22,.25,-.08,-1.25,gunDark);}
      else{box(.32,.25,.9,.25,-.28,-.78);box(.25,.21,.58,.25,-.26,-1.48,gunDark);tube(.045,.72,.25,-.25,-2.1);box(.18,.45,.2,.25,-.5,-.82,gripMat).rotation.x=-.18;box(.14,.38,.22,.25,-.45,-1.08,gunDark).rotation.x=.18;box(.13,.08,.18,.25,-.1,-.6,gunDark);}
      const sightZ=key==="SCOUT"?-.55:key==="HEAVY"?-.42:-.38,sightSize=key==="HEAVY"?.075:key==="LIGHT"?.065:.085;const sight=new THREE.Mesh(new THREE.TorusGeometry(sightSize,.012,8,18),gunDark);sight.position.set(.25,-.1,sightZ);sight.userData.sight=true;w.add(sight);const sightDot=new THREE.Mesh(new THREE.SphereGeometry(.008,6,4),new THREE.MeshBasicMaterial({color:0xe65b43}));sightDot.position.set(.25,-.1,sightZ+.006);sightDot.userData.sight=true;w.add(sightDot);
      const rearHand=new THREE.Mesh(new THREE.SphereGeometry(.13,10,8),handMat);rearHand.position.set(.25,-.42,-.5);rearHand.scale.set(.9,1.15,.9);rearHand.userData.hands=true;w.add(rearHand);const supportHand=new THREE.Mesh(new THREE.SphereGeometry(.13,10,8),handMat);supportHand.position.set(-.02,-.37,-1.18);supportHand.scale.set(1.1,.85,1);supportHand.userData.hands=true;w.add(supportHand);const sleeveA=new THREE.Mesh(new THREE.CapsuleGeometry(.13,.46,4,8),sleeveMat);sleeveA.position.set(.36,-.58,-.2);sleeveA.rotation.x=.75;sleeveA.userData.hands=true;w.add(sleeveA);const sleeveB=sleeveA.clone();sleeveB.position.set(-.18,-.56,-.83);sleeveB.rotation.x=1.05;w.add(sleeveB);w.position.set(.1,.03,0);return w;};
    for(const key of CLASS_KEYS){weaponModels[key]=makeViewWeapon(key);weaponModels[key].visible=false;gun.add(weaponModels[key]);}camera.add(gun);scene.add(camera);
    ...
    const shoot=()=>{if(!playing||reloading)return;const spec=CLASSES[selectedRef.current];if(performance.now()-lastShot<spec.rate||ammo<=0)return;lastShot=performance.now();ammo--;shotSound();gun.position.z=.08;ray.setFromCamera(new THREE.Vector2(0,0),camera);ray.far=spec.range;const hits=ray.intersectObjects(bots.filter(b=>b.team==="RED").map(b=>b.mesh),true);if(hits.length){const bot=bots.find(b=>{let o:THREE.Object3D|null=hits[0].object;while(o?.parent&&o.parent!==scene)o=o.parent;return o===b.mesh;});if(bot){bot.hp-=spec.damage;bot.bar.scale.x=Math.max(.01,bot.hp/200);bot.bar.position.x=-(1-bot.hp/200)*.58;if(bot.hp<=0){scene.remove(bot.mesh);const idx=bots.indexOf(bot);bots.splice(idx,1);setTimeout(()=>{if(playing) spawnOne("RED")},1800);}}}if(ammo===0)reload();};
    function reload(){if(reloading)return;reloading=true;reloadStarted=performance.now();setTimeout(()=>{ammo=selectedRef.current==="HEAVY"?100:selectedRef.current==="SCOUT"?5:30;reloading=false},1300);}
    function spawnOne(team:"RED"|"BLUE"){const cm=botMesh(team,CLASS_KEYS[Math.floor(Math.random()*5)]);cm.g.position.set(team==="RED"?-72:72,0,(Math.random()-.5)*65);scene.add(cm.g);bots.push({mesh:cm.g,bar:cm.bar,team,hp:200,target:points[Math.floor(Math.random()*3)].pos.clone(),cooldown:1,skill:difficultyRef.current,classKey:CLASS_KEYS[Math.floor(Math.random()*5)]});}

    const down=(e:KeyboardEvent)=>{keys.add(e.code);if(e.code==="KeyR")reload();if(e.code==="Space"&&onGround){velocityY=7;onGround=false}};const up=(e:KeyboardEvent)=>keys.delete(e.code);
    ...
    function animate(now:number){requestAnimationFrame(animate);const dt=Math.min(.035,(now-prev)/1000);prev=now;gun.position.z*=.78;const classKey=selectedRef.current,sniperAiming=playing&&aiming&&classKey==="SCOUT",targetFov=aiming?(sniperAiming?24:43):72;if(Math.abs(camera.fov-targetFov)>.05){camera.fov+=(targetFov-camera.fov)*Math.min(1,dt*13);camera.updateProjectionMatrix();}const adsAmount=Math.min(1,dt*14);gun.position.x+=((aiming&&!sniperAiming?-.35:0)-gun.position.x)*adsAmount;gun.position.y+=((aiming&&!sniperAiming?.07:0)-gun.position.y)*adsAmount;gun.visible=playing&&!sniperAiming;for(const key of CLASS_KEYS)weaponModels[key].visible=key===classKey;weaponModels[classKey].traverse(o=>{if((o as THREE.Mesh).isMesh)o.visible=aiming?Boolean(o.userData.sight):true});weaponRig.style.display="none";sniperScope.style.display=sniperAiming?"block":"none";root.parentElement?.classList.toggle("sniper-aiming",sniperAiming);if(playing){
      ...
      bots.forEach(bot=>{if(bot.hp<=0)return;const enemy=bot.team==="RED"?player:null;const toTarget=bot.target.clone().sub(bot.mesh.position);const nearPlayer=bot.team==="RED"&&bot.mesh.position.distanceTo(player)<48;if(nearPlayer&&enemy){toTarget.copy(player).sub(bot.mesh.position)}toTarget.y=0;if(toTarget.length()>3){bot.mesh.position.addScaledVector(toTarget.normalize(),dt*(bot.classKey==="LIGHT"?5.2:3.7));bot.mesh.lookAt(bot.mesh.position.clone().add(toTarget));}bot.bar.lookAt(camera.position);bot.cooldown-=dt;if(nearPlayer&&bot.cooldown<=0){const d=DIFFICULTIES[bot.skill];bot.cooldown=d.fire+Math.random()*.5;const dist=bot.mesh.position.distanceTo(player);const hitChance=Math.max(.08,.84-dist*.012-d.aim);if(Math.random()<hitChance){hp-=bot.classKey==="SCOUT"?34:bot.classKey==="HEAVY"?12:20;shake=.13;hitFlash=1;if(hp<=0){hp=0;playing=false;document.exitPointerLock();setScreen("dead");}}}});
